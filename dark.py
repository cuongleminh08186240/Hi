...
ADMIN_IDS = [6454123620, 6454123620]
BOT_TOKEN = "8013444951:AAEP8uW8OabtHZhiwcpeQvZ5Moai-j0ZJQI"
thread_count = 900
packet_size = 9
...
def load_admin_data():
    ...
    try:
        ...
        logger.error(f"Lỗi tải dữ liệu quản trị viên: {e}")
        return {'admins': {str(admin_id): {'balance': float('inf')} for admin_id in ADMIN_IDS}}
...
def save_admin_data(data):
    ...
    try:
        ...
        logger.error(f"Lỗi khi lưu dữ liệu quản trị: {e}")
        return False
...
def load_users():
    ...
    try:
        ...
        logging.error(f"Lỗi tải người dùng: {e}")
        return []
...
def save_users(users):
    ...
    try:
        ...
        logging.error(f"Lỗi lưu người dùng: {e}")
...
def load_keys():
    ...
    try:
        ...
        logging.error(f"Lỗi tải khóa: {e}")
        return {}
...
def save_keys(keys: Dict[str, timedelta]):
    ...
    try:
        ...
        logging.error(f"Lỗi lưu chìa khóa: {e}")
        if os.path.exists(temp_file):
            os.remove(temp_file)
        return False
...
def check_user_expiry():
    ...
    while True:
        try:
            ...
            logging.error(f"Lỗi trong check_user_expiry: {e}")
...
def set_thread_count(message):
    ...
    if not is_super_admin(user_id):
        bot.send_message(chat_id, "*Bạn không được phép thay đổi cài đặt chủ đề.*", parse_mode='Markdown')
        return

    bot.send_message(chat_id, "*Vui lòng chỉ rõ số lượng sợi.*", parse_mode='Markdown')
...
def set_packet_size(message):
    ...
    if not is_super_admin(user_id):
        bot.send_message(chat_id, "*Bạn không được phép thay đổi cài đặt kích thước gói.*", parse_mode='Markdown')
        return

    bot.send_message(chat_id, "*Vui lòng chỉ định kích thước gói.*", parse_mode='Markdown')
...
def process_packet_command(message):
    ...
    try:
        ...
        if new_packet_size <= 0:
            bot.send_message(chat_id, "*Kích thước gói phải là số dương.*", parse_mode='Markdown')
            return

        packet_size = new_packet_size
        bot.send_message(chat_id, f"*Kích thước gói được đặt thành {packet_size} cho dark*", parse_mode='Markdown')

    except ValueError:
        bot.send_message(chat_id, "*Kích thước gói không hợp lệ. Vui lòng nhập số hợp lệ..*", parse_mode='Markdown')
...
def process_thread_command(message):
    ...
    try:
        ...
        if new_thread_count <= 0:
            bot.send_message(chat_id, "*Số lượng luồng phải là số dương.*", parse_mode='Markdown')
            return

        thread_count = new_thread_count
        bot.send_message(chat_id, f"*Số lượng luồng được đặt thành {thread_count} cho chế độ tối.*", parse_mode='Markdown')

    except ValueError:
        bot.send_message(chat_id, "*Số lượng luồng không hợp lệ. Vui lòng nhập số lượng hợp lệ..*", parse_mode='Markdown')
...
def attack_command(message):
    ...
    if is_admin(user_id):
        try:
            bot.send_message(chat_id, "*Vui lòng nhập ✅:\n<𝐈𝐏> <𝐏𝐎𝐑𝐓> <𝐓𝐈𝐌𝐄>.*", parse_mode='Markdown')
...
def process_attack_command(message, chat_id):
    try:
        ...
        if len(args) != 3:
            bot.send_message(chat_id, "*Có gì đó không ổn. Hãy thử lại 😼*", parse_mode='Markdown')
            return
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By ᚛ @Cuonghackgames ᚜"  
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! Developed By ᚛ @Cuonghackgames ᚜\n\n"
            f"Admin Commands:\n"
        ...
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! Developed By ᚛ @Cuonghackgames ᚜\n\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Cuonghackgames ᚜*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Cuonghackgames ᚜.*", parse_mode='Markdown')
        return
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                ...
                "Contact ᚛ @Cuonghackgames ᚜ for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Cuonghackgames ᚜."
